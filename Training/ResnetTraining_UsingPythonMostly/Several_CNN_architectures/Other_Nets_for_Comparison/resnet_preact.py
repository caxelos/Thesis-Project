### RUN MODEL WITH:
#python -u main.py --arch resnet_preact --dataset data --test_id 0 --outdir results/resnet_preact/00 --batch_size 32 --base_lr 0.1 --momentum 0.9 --nesterov True --weight_decay 1e-4 --epochs 40 --milestones '[30, 35]' --lr_decay 0.1 --tensorboard
...
class Model(nn.Module):
    def __init__(self):
        ...
        n_blocks_per_layer = (depth - 2) // 6
        assert n_blocks_per_layer * 6 + 2 == depth

        n_channels = [base_channels, base_channels * 2, base_channels * 4]

        #in_channels=1, out_channels=16
        self.conv = nn.Conv2d(
            input_shape[1],#(1,1,36,60)
            n_channels[0],#16
            kernel_size=(3, 3),
            stride=1,
            padding=1,
    ...
    def _make_layer(self, in_channels, out_channels, n_blocks, block, stride):

        layer = nn.Sequential()#https://pytorch.org/docs/stable/nn.html#sequential
