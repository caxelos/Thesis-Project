...
for i = 1:140 %for each tree
	...
	for i = 1:140 
	   ...
	   if i == 128 || i == 32 || i == 129 || i == 91 || i == 130 || i == 126
	      ...
	      trees(i) = buildRegressionTree( samplesInTree(i), i, treeImgs(i,:,:,:),  treeGazes(i,:,:), HEIGHT, WIDTH, trees(i), 1 );
...
function trees = buildRegressionTree( fatherSize, treeNum, treeImgs,  treeGazes, HEIGHT, WIDTH, trees, node_i)
	...
	minSquareError1 = zeros(3); % a huge value
	minPx1_vert1 =     zeros(3); % something random here
	minPx1_hor1 =     zeros(3); % also here
	minPx2_vert1=     zeros(3); % and here..
	minPx2_hor1 =     zeros(3); % and here 
	bestThres1 =     zeros(3); % ah, and here
	%%% recursion staff %%%
   ...
   while 1 %3
	...
	%for each node
	for j =1:3
	minSquareError1(j) = 10000; % a huge value
	minPx1_vert1(j) =    10000; % something random here
	minPx1_hor1(j) =     10000; % also here
	minPx2_vert1(j)=     10000; % and here..
	minPx2_hor1(j) =     10000; % and here 
	bestThres1(j)  =     10000; % ah, and here
	end
...
%	   for px1_hor = 1:WIDTH
		...
		for px2_vert = ( px1_vert + floor(px1_hor/WIDTH)  ):HEIGHT
		  for px2_hor = (1 + mod( px1_hor, WIDTH )):WIDTH
			...
			for j = 1:fatherSize
			   ...
			   if  abs(treeImgs(treeNum, currPtrs(j), px1_vert, px1_hor) - treeImgs(treeNum, currPtrs(j),px2_vert, px2_hor))  < thres 
			      %left child

			      l = l + 1;
			      ltree_tempGazes(l,w ) = treeGazes(treeNum, currPtrs(j) );
			      lImgs(w,l) = currPtrs(j); 
			       				      
			      
			      meanLeftGaze(1) = meanLeftGaze(1) + treeGazes(treeNum,currPtrs(j),1);%,:);
			      meanLeftGaze(2) = meanLeftGaze(2) + treeGazes(treeNum,currPtrs(j),2);%,:);	
			      ...
			      r = r + 1;
			      rtree_tempGazes(r) = treeGazes(treeNum,currPtrs(j) );
			      rImgs(r) = currPtrs(j);  				      
			      
			      meanRightGaze(1) = meanRightGaze(1) + treeGazes(treeNum,currPtrs(j),1);%,:);
			      meanRightGaze(2) = meanRightGaze(2) + treeGazes(treeNum,currPtrs(j),2);
			   end
			end
			...
			if squareError < minSquareError
			   minSquareError1(w) = squareError;			
			   minPx1_vert1(w) =    px1_vert; % something random here
			   minPx1_hor1(w)  =     px1_hor; % also here
			   minPx2_vert1(w)=     px2_vert; % and here..
			   minPx2_hor1(w) =     px2_hor; % and here
			   bestThres1(w)  =     thres;
			   
			   
			   for o = 1:r
			      final_rImgs(w,o) = rImgs(o);%%%%%%%%%%%%
			   end

			   for o = 1:l
			      final_lImgs(w,o) = lImgs(o);%%%%%%%%%%%%
			   end				
	...
	minSquareError =  minSquareError1(j); 
	minPx1_vert = minPx1_vert1(j);  
	minPx1_hor = minPx1_hor1(j);     
	minPx2_vert = minPx2_vert1(j);     
	minPx2_hor  =minPx2_hor1(j);      
	bestThres = bestThres1(j);      
